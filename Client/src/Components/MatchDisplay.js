...
const MatchDisplay=({matches , setClickedUser})=>{
    ...
    const filteredMatchedProfile=matchedProfiles?.filter((matchedprofile)=>matchedprofile.matches.some((profile)=>profile.user_id==userID))

    return(
        <>
            <div className="matchesdisplay">
                <div > {filteredMatchedProfile?.map((users,_index) => (
